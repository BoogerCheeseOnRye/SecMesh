...
function resize(){
  // In crossWall mode the lab wrap is display:none; keep the renderer buffer
  // sized to the overlay (which is the full viewport) so tile blits stay crisp.
  const { w, h } = crossWallState.on ? wallSize() : { w: wrap.clientWidth, h: wrap.clientHeight };
...
function wallSize(){
  // #crossWall is position:fixed;inset:0, so it spans the full viewport. The
  // lab wrap is display:none during crossWall mode, so measuring it there
  // returns 0 and collapsed the wall into one column of stretched strips.
  const ov = document.getElementById('crossWall');
  const w = (ov && ov.clientWidth) || window.innerWidth || 800;
  const h = (ov && ov.clientHeight) || window.innerHeight || 600;
  return { w, h };
}

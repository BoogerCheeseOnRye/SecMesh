...
## Taskbar control & resource-aware monitoring

`app/serve.js` (the static server) is also the **service manager for the mesh**. On
boot it brings up the self-healing `node-tests/peer-supervisor.mjs` fabric and a
**taskbar icon** for the IceWM SystemTray:

- **Left-click** → open the defense console.
- **Right-click** → menu: `Status…`, `Reboot SecMesh`, `Turn Off SecMesh`,
  `Start SecMesh`, `Quit Tray Icon`.
- The icon is **live**: green `on` = fabric up, gray `off` = stopped,
  orange `warn` = supervisor down/unreachable. It re-polls every ~2 s and swaps
  on state changes (an older-version quirk — orphaned duplicate icons — is
  handled by reaping every yad carrying the icon path before each redraw).

Control is a tiny HTTP API on the same server (also what the tray menu calls):

```
POST/GET  /peerctl?action=status     → {up, pairs, devices, chassis (with battery), stats}
          /peerctl?action=stop       → kills phone+host peers + supervisor; serve.js+icon stay
          /peerctl?action=restart    → full stop, then a fresh self-healing fabric
          /peerctl?action=launch     → (re)start the fabric (idempotent)
```

While `stopped`, `status` returns `503 {"ok":false,"stopped":true,...}` and will
**not** auto-revive (no surprise network activity); the tray Start button or
`?action=launch` resumes it.

### Battery-aware throttle (phones)

The supervisor polls each attached device's battery (30 s cache TTL) and **parks**
a chassis when its level drops to `BAT_PARK` (default **20 %**) while unplugged —
its needle/executor peers are killed and respawned only after charging returns to
`BAT_RESUME` (default **35 %**). Parked chassis report `parked:true` plus
`battery:{level,charging}` in `/peerctl` status (shown in the tray Status dialog),
so planning spreads load across the still-healthy devices instead of draining the
weakest.

### Wiring

- `app/secmesh-tray.sh <port>` — the tray daemon (heartbeat pidfile, single
  instance, no orphans).
- `app/secmesh-status.sh <port>` — Status dialog (Pango-formatted live summary).
- `app/secmesh-ctl.sh <start|stop|reboot> <port>` — tray menu action → toast.
- `app/assets/secmesh-{on,off,warn}.png` — 24×24 icons, generated by
  `app/make-icons.py` (stdlib-only PNG writer; no ImageMagick on this box).
- **Autostart** (optional, mirrors the cluster-flag pattern): guarded by a flag
  file so a login never surprises you —
  `touch ~/.secmesh-enabled` (enable) · `rm -f ~/.secmesh-enabled` (disable).
